##############################
# coding: utf-8
# use like > python morph2-AD-LC.py --cuda 0
##############################
# Imports
##############################
import os
...
for RANDOM_SEED in [0]:
    ...
    for epoch in range(NUM_EPOCHS):
        ...
        with torch.set_grad_enabled(False):
            tra_NN_Z,  tra_NN_A,  tra_NN_S  = compute_errors(model, train_loader, 'NN')
            ...
            #
            val_NN_Z,  val_NN_A,  val_NN_S  = compute_errors(model, valid_loader, 'NN')
            val_ROT_Z, val_ROT_A, val_ROT_S = compute_errors(model, valid_loader, 'ROT', False, V_Z, V_A, V_S)
            #
            tes_NN_Z,  tes_NN_A,  tes_NN_S  = compute_errors(model, test_loader, 'NN')
            tes_ROT_Z, tes_ROT_A, tes_ROT_S = compute_errors(model, test_loader, 'ROT', False, V_Z, V_A, V_S)
        # SAVE CURRENT/BEST ERRORS/TIME
        s = '%f,%f,%f,%f,%f,%f,%f,%f,%f,%f,%f,%f,%f,%f,%f,%f,%f,%f,%d,%d,%d' % ( 
            tra_NN_Z, tra_NN_A, tra_NN_S, tra_ROT_Z, tra_ROT_A, tra_ROT_S, 
            val_NN_Z, val_NN_A, val_NN_S, val_ROT_Z, val_ROT_A, val_ROT_S, 
            tes_NN_Z, tes_NN_A, tes_NN_S, tes_ROT_Z, tes_ROT_A, tes_ROT_S, 
            vz_ord, va_ord, vs_ord)
        print(s)
