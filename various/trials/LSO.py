...
dataname_set = [['EF3-2d-planes','EF3-abalone','EF3-ailerons','EF3-auto-mpg','EF3-auto-price','EF3-bank-domain-1','EF3-bank-domain-2','EF3-boston-housing','EF3-california-housing','EF3-census-1','EF3-census-2','EF3-computer-activity-1','EF3-computer-activity-2','EF3-delta-ailerons','EF3-delta-elevators','EF3-diabetes','EF3-elevators','EF3-friedman-artificial','EF3-kinematics-of-robot-arm','EF3-machine-cpu','EF3-mv-artificial','EF3-pole-telecomm','EF3-pumadyn-domain-1','EF3-pumadyn-domain-2','EF3-pyrimidines','EF3-servo','EF3-stock-domain','EF3-triazines','EF3-wisconsin-breast-cancer'], 
                ...
                ['contact-lenses','pasture','squash-stored','squash-unstored','tae','newthyroid','balance-scale','SWD','car','bondrate','toy','eucalyptus','LEV','automobile','winequality-red','ESL','ERA']]
...
def learning(seed, train_data, valid_data, test_data, node, epoch):
    ...
    #loop epoch
    if method=="ODOB_LogiAT": M = method.split('_')[1]
    elif method=="POOCL_NLL": M = method
    #
    res = np.zeros((epoch,27))
    for e in range(epoch):
        eval("MF.train_"+M)(train_loader, classnum, device, model, optimizer)
        #
        all_g, all_y = MF.B_all_gy(train_loader2, device, model)
        ...
        res[e, 0: 3] = eval("MF.test_"+M)(train_loader2, classnum, "O", device, model, "RL", False, None, None, None)
        res[e, 3: 6] = eval("MF.test_"+M)(train_loader2, classnum, "O", device, model, "ST", False, None, None, None)
        res[e, 6: 9] = eval("MF.test_"+M)(train_loader2, classnum, "O", device, model, "OT", False, t_Z, t_A, t_S)
        #
        res[e, 9:12] = eval("MF.test_"+M)(valid_loader, classnum, "O", device, model, "RL", False, None, None, None)
        res[e,12:15] = eval("MF.test_"+M)(valid_loader, classnum, "O", device, model, "ST", False, None, None, None)
        res[e,15:18] = eval("MF.test_"+M)(valid_loader, classnum, "O", device, model, "OT", False, t_Z, t_A, t_S)
        #
        res[e,18:21] = eval("MF.test_"+M)(test_loader,  classnum, "O", device, model, "RL", False, None, None, None)
        res[e,21:24] = eval("MF.test_"+M)(test_loader,  classnum, "O", device, model, "ST", False, None, None, None)
        res[e,24:27] = eval("MF.test_"+M)(test_loader,  classnum, "O", device, model, "OT", False, t_Z, t_A, t_S)
