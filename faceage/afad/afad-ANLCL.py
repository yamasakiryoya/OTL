##############################
# coding: utf-8
# use like > nohup python afad-ANLCL.py --cuda 0 &
##############################
# Imports
##############################
import os
...
for RANDOM_SEED in range(20):
    ...
    PATH = "MTM/ANLCL/seed"+str(RANDOM_SEED)
    ...
    valid_dataset = AFAD_Dataset(df=VALID_DF, img_dir=IMAGE_PATH, transform=custom_transform2)
    test_dataset  = AFAD_Dataset(df=TEST_DF,  img_dir=IMAGE_PATH, transform=custom_transform2)
    ...
    for epoch in range(NUM_EPOCHS):
        ...
        s = 'MZE/MAE/RMSE | Current : %.4f/%.4f/%.4f/%.4f/%.4f/%.4f/%.4f/%.4f/%.4f/%.4f/%.4f/%.4f Ep. %d Ord. %d/%d/%d/%d | Best-LB : %.4f/%.4f/%.4f Ep. %d/%d/%d | Best-MT : %.4f/%.4f/%.4f Ep. %d/%d/%d | Best-ST : %.4f/%.4f/%.4f Ep. %d/%d/%d | Best-IOT : %.4f/%.4f/%.4f Ep. %d/%d/%d' % ( 
            LB_Z, LB_A, LB_S, MT_Z, MT_A, MT_S, ST_Z, ST_A, ST_S, IOT_Z, IOT_A, IOT_S, epoch, b_ord, vz_ord, va_ord, vs_ord,
