##############################
# coding: utf-8
# use like > nohup python afad-ord-hing-IT.py --cuda 0 &
##############################
# Imports
##############################
import os
...
for RANDOM_SEED in range(10):
    ...
    #
    PATH = "result/ord-hing-IT/seed"+str(RANDOM_SEED)
    ...
    def loss_fn(a, b, targets):
        tmpl = torch.cat([torch.zeros(a.shape[0],1).to(DEVICE),torch.relu(1.+b-a)],dim=1).gather(dim=-1, index=targets.unsqueeze(1)).squeeze(1)
        tmpr = torch.cat([torch.relu(1.-b+a),torch.zeros(a.shape[0],1).to(DEVICE)],dim=1).gather(dim=-1, index=targets.unsqueeze(1)).squeeze(1)
